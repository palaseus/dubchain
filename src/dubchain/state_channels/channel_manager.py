...
class ChannelManager:
    ...
    def __init__(self, config: Optional[ChannelConfig] = None):
        ...
        # 3. Verifying contract deployment success
        # 4. Storing contract address in configuration
        self.on_chain_contract = OnChainContract(self._get_contract_address())
        ...
        self.participant_channels: Dict[
            str, Set[ChannelId]
        ] = {}  # participant -> set of channels
    ...
    def create_channel(
        ...
        try:
            ...
            if len(participants) > self.config.max_participants:
                errors.append(
                    f"Too many participants: {len(participants)} > {self.config.max_participants}"
                )
            ...
            for participant in participants:
                if participant not in deposits:
                    ...
                    errors.append(
                        f"Deposit too small for {participant}: {deposits[participant]} < {self.config.min_deposit}"
                    )
            ...
            self.on_chain_contract.register_channel(
                channel_id, participants, deposits, config
            )
    ...
    def update_channel_state(
        ...
        try:
            ...
            (
                security_valid,
                security_events,
            ) = self.security_manager.validate_channel_operation(
                channel_id,
                "state_update",
                {
                    "update": update,
            ...
            if not security_valid:
                ...
                return False, [
                    f"Security violation: {event.threat_type.value}"
                    for event in security_events
                ]
    ...
    def close_channel(
        ...
        try:
            ...
            # Close the channel
            close_reason = (
                ChannelCloseReason.COOPERATIVE
                if reason == "cooperative"
                else ChannelCloseReason.DISPUTE
            )
    ...
    def get_channel_info(self, channel_id: ChannelId) -> Optional[Dict[str, Any]]:
        ...
        if channel_id not in self.active_channels:
            ...
            if state:
                return {
                    ...
                    "close_reason": state.close_reason.value
        ...
        info["active_disputes"] = len(
            [
                d
                for d in disputes
                if d.status.value in ["pending", "evidence_period", "challenge_period"]
            ]
        )
    ...
    def get_global_metrics(self) -> Dict[str, Any]:
        ...
        total_security_events = sum(
            m.security_events for m in self.channel_metrics.values()
        )
        total_disputes = sum(
            m.disputes_initiated for m in self.channel_metrics.values()
        )
        ...
        update_times = [
            m.average_update_time
            for m in self.channel_metrics.values()
            if m.average_update_time > 0
        ]
        average_update_time = (
            sum(update_times) / len(update_times) if update_times else 0
        )
                ...
                for channel_id, channel in self.active_channels.items():
                    ...
                    if channel_state:
                        ...
                        if self.config.enable_timeout_mechanism:
                            time_since_last = (
                                current_time - channel_state.last_update_timestamp
                            )
                            if (
                                time_since_last > self.config.timeout_blocks * 12
                            ):  # Assume 12s per block
    ...
    def _get_contract_address(self) -> str:
        """
        Get the deployed state channel contract address.

        TODO: Implement actual contract address retrieval
        This would involve:
        1. Checking if contract is already deployed
        2. Deploying contract if not found
        3. Retrieving address from deployment transaction
        4. Storing address in persistent storage
        """
