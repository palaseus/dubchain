...
class BatchVerifier:
    ...
    def verify_batch(
        self,
        verify_func: Callable[[Proof, List[bytes]], VerificationResult],
        proofs: List[Proof],
        public_inputs_list: List[List[bytes]],
        ...
        for batch_proofs, batch_inputs in batches:
            batch_results = self._verify_batch_parallel(
                verify_func, batch_proofs, batch_inputs
            )
            results.extend(batch_results)

        return results

    def _verify_batch_parallel(
        self,
        verify_func: Callable[[Proof, List[bytes]], VerificationResult],
        proofs: List[Proof],
        public_inputs_list: List[List[bytes]],
...
class ProofVerifier:
    ...
    def verify_with_timeout(
        self, verify_func: Callable[[], VerificationResult]
    ) -> VerificationResult:
